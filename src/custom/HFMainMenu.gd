...
func on_button_pressed(_button_name : String) -> void:
	match _button_name:
		"SinglePlayerDemo":
			# warning-ignore:return_value_discarded
			get_tree().change_scene_to_file(CFConst.PATH_CUSTOM + 'HFMain.tscn')
		"Multiplayer":
			pass
		"GUT":
			# warning-ignore:return_value_discarded
...
#			main_position_x = -get_viewport().size.x
	...
	tween.tween_property(main_menu, 'position:x',
			main_position_x, menu_switch_time)\
			.from(main_menu.position.x)\
			.set_trans(Tween.TRANS_BACK).set_ease(Tween.EASE_IN_OUT)
	tween.tween_property(tab, 'position:x',
			0, menu_switch_time)\
			.from(tab.position.x)\
			.set_trans(Tween.TRANS_BACK).set_ease(Tween.EASE_IN_OUT)
...
#			tab_position_x = get_viewport().size.x
	...
	var tween = create_tween().set_parallel(true)
	tween.tween_property(tab, 'position:x',
			tab_position_x, menu_switch_time)\
			.from(tab.position.x)\
			.set_trans(Tween.TRANS_BACK).set_ease(Tween.EASE_IN_OUT)
	tween.tween_property(main_menu, 'position:x',
			0, menu_switch_time)\
