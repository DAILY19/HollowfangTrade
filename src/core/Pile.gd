...
func _on_ViewPopup_about_to_show() -> void:
	...
	vp_tween.tween_property($ViewPopup, 'modulate', Color(1,1,1,1), 0.5)\
			.from(Color(1,1,1,0))\
...
func _on_ViewPopup_popup_hide() -> void:
	var vp_tween = create_tween()
	vp_tween.tween_property($ViewPopup, 'modulate', Color(1,1,1,0), 0.5)\
			.from(Color(1,1,1,1))\
			.set_trans(Tween.TRANS_EXPO).set_ease(Tween.EASE_OUT)
	await vp_tween.finished
