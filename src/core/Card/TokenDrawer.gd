...
func token_drawer(requested_state := true) -> void:
	...
	if (_tween == null or not _tween.is_running()) and \
			(owner_card._flip_tween == null or not owner_card._flip_tween.is_running()) and \
			(owner_card._tween == null or not owner_card._tween.is_running()):
		...
		if requested_state == true and $Drawer/VBoxContainer.get_child_count():
			...
			_tween.tween_property(
					td,'position',
					Vector2(owner_card.card_size.x - 35, td.position.y),
					0.2).from(td.position)\
