	...
	for task in scripts_queue:
		...
		if script.is_primed:
			...
			if script.script_name == "custom_script"\
				...
				if retcode is Signal:
					retcode = await retcode
				...
				if costs_dry_run():
					if retcode != CFConst.ReturnCode.CHANGED:
						...
						if confirm_return is Signal: # Still working.
							await confirm_return
...
func spawn_card_to_container(script: ScriptTask) -> void:
	...
	if card_filters:
		...
		if selection_amount == 1:
			...
			if select_return is Signal: # Still working.
				select_return = await select_return
