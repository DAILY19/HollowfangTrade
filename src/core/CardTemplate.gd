...
@export var card_rotation: int = 0: set = set_card_rotation, get = get_card_rotation
...
func _process(delta) -> void:
	if _tween != null and _tween.is_running() and not cfc.ut: # Debug code for catch potential Tween deadlocks
...
func set_card_rotation(
	...
	# and a toggle was not requested, we consider we did nothing
		...
		if not state in [CardState.PREVIEW, CardState.DECKBUILDER_GRID]\
				...
				and not (_tween != null and _tween.is_running()):
...
func execute_scripts(
	...
	var confirm_return = CFUtils.confirm(
		...
		trigger,
		state_exec)
	confirm_return = await confirm_return
...
func _organize_attachments() -> void:
	...
	if get_parent() == cfc.NMAP.board:
		for card in attachments:
			...
			if (card._tween == null or not card._tween.is_running()) and \
					card.state in \
...
func _flip_card(to_invisible: Control, to_visible: Control, instant := false) -> void:
	...
	# So we skip that.
		...
		_flip_tween.tween_property(highlight, 'position',
				Vector2((highlight.size.x-3)/2,0), 0.4)\
				.from(highlight.position)\
				.set_trans(Tween.TRANS_QUAD).set_ease(Tween.EASE_IN)
...
#			print(global_position + _control.rect_size)
			...
			if not (_tween != null and _tween.is_running()) and \
...
#				resize_recursively(highlight, 1.0)
		...
		CardState.DROPPING_TO_BOARD:
			...
			if not (_tween != null and _tween.is_running()):
				if _tween: _tween.kill() # We make sure to remove other tweens of the same type to avoid a deadlock
#				_target_position = _determine_board_position_from_mouse()
#				# The below ensures the card doesn't leave the viewport dimentions
